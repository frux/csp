...
describe("CSP building", () => {
	...
	test("should correctly make policy with many directives", () => {
		expect(
			...
			"script-src host1.com host2.com; style-src host1.com host2.com;"
		);
	});
});
...
describe("Validation", () => {
	...
	test("should filter empty directives", () => {
		expect(
			getCSP({
				directives: {
					"script-src": ["test1", false],
				},
				presets: [
					{
						"style-src": ["test2", false],
					},
				],
			})
		).toBe("script-src test1; style-src test2;");
	...
	describe("Array format", () => {
		...
		test("should remove duplicating rules", () => {
			expect(
				...
				"script-src domain1.com domain2.com; style-src domain1.com domain2.com;"
			);
		});
